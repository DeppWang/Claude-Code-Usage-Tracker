...
工具依赖定时任务采集数据，电脑如果休眠了，定时任务就不会执行，数据也就采集不了。所以要么当前设备保持不休眠，要么用一台额外的设备来跑 Menu Bar 应用。不过如果额外的设备本身不用 Claude Code，Quota 数据也算不出来——但不用也就不需要监控了。
